...
def sgd(cost, params, lr=0.005):
    ...
    # [(state, state + inc)]
    updates = [(params, params - grads * lr)]
    return updates
...
def mlp_synthetic(X_train, X_test, y_train, y_test, precision, vy, hWidths, mini_batchsize=10, epochs=1000,
                  display=False):
    ...
    if (display):
        print 'MSE for mean prediction, Train:{} ,Test:{}'.format(mean_pred_train_cost, mean_pred_test_cost)
